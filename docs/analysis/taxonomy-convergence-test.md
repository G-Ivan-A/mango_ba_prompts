...
| Тестируемый реестр | [`kb/industry/reference-taxonomy.json`](../../kb/industry/reference-taxonomy.json) (`version: 1.1.0`, `status: active`) |
| Источник функций | [`kb/mango/mango-registry.json`](../../kb/mango/mango-registry.json) (160 атомарных функций) + processed-документация |
...
- **Rater A (эталон):** для каждой функции взят документированный `industry_ref`
  из `kb/mango/mango-registry.json`. Этот mapping строился ранее, в отдельных
...
- **Rater B (AI-агент под тестом):** изолированный subagent, которому выданы
  **только** два артефакта — `standards/industry-taxonomy-standard.md` и
  `kb/industry/reference-taxonomy.json` — плюс список из 25 функций (русское имя,
  тонкое описание, имя продуктового модуля). Агенту **запрещено** открывать
  `kb/mango/`, `docs/`, `experiments/`, mango-taxonomy-standard и искать в вебе.
...
change request к `kb/mango/mango-registry.json`** (вне рамок данной задачи —
...
7. Исправить субоптимальные mapping'и в `kb/mango/mango-registry.json`,
