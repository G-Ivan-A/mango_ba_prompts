...
4. Зафиксировать специфику Конарда:
   - Фаундер молчит + мержит PR → молчание = согласие;
...
3. Acceptance: Фаундер мержит без комментариев (молчание = согласие), либо
   комментирует и вручную перезапускает задачу, либо закрывает PR.

Локальная проверка этой задачи: `python3 experiments/validate_issue_61_governance.py`.
...
- **Всегда спрашивать Фаундера перед ADR вне scope.** Отклонено для Creative:
