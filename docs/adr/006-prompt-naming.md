...
**Пример 1 — детерминированный разбор.**
`technical-details-solution-design-legacy` → домен `technical-details`,
операция `solution-design`, режим `legacy` (парс-алгоритм §1; операция входит в
таксономию 13 операций, [ADR-004](004-operations-taxonomy.md)).

**Пример 2 — отклонение перегрузки (P1).** Имя `fr-validation-kb-stepwise`
вводит четвёртую ось (`kb`) и **отклоняется**: KB-функция выражается общим блоком
(§3), а не осью имени. Аналогично архивные `…-generator`/`…-simple` нарушают P1 и
служат антипримером (§4).

**Пример 3 — полный реестр (доказательство ФТ-4).** Все 24 активных промпта
разобраны в таблице §4 как `<домен>-<операция>-<режим>`; ни одно имя не
перегружено.
