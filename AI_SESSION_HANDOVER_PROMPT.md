...
не копируя весь текст шаблона в чат; если шаблона нет — используй формат задачи для
Конарда из Хаба (templates/task-for-konard.md). Задачу для Конарда оформляй по полям:
Тип / Файл / Шаблон / Содержимое / Дополнительно (issue, приоритет).
...
3. **Если шаблона нет** — используй стандартный формат задачи для Конарда из Хаба
   (`templates/task-for-konard.md`) или предложи создать шаблон.
4. **Подтверди с пользователем:** «Подтверждаете использование шаблона [название]?»

## 📝 Формат постановки задач для Конарда

Исходный формат — `templates/task-for-konard.md` в Хабе
([hybrid-Intelligence-lab](https://github.com/G-Ivan-A/hybrid-Intelligence-lab)). Перед
формулировкой проверь наличие шаблонов в репо и ссылайся на них, не копируя весь текст
в чат.

~~~text
## Задача для Конарда
**Тип:** Создание файла / Изменение файла / Создание PR
**Файл:** `путь/к/файлу.md`
**Шаблон:** [ссылка на шаблон, если есть] / «Нет шаблона»
