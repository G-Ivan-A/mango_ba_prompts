...
- Добавлен анализ первой реальной сессии БА
  ...
  вердикты по 4 гипотезам БА с цитатами из стенограммы, конкретные правки
  промптов (P1–P5) и рекомендации по онтологии (без её изменения).
...
### Changed — Issue #101 правки промптов по итогам эксперимента 1027 (`0.1 → 0.2`)

- `glossary-context-understanding-stepwise.md`, `…-oneshot.md`: документация по
  продукту явно объявлена источником верификации As-Is, а не источником скоупа;
  Шаг 0 явно запрашивает документацию/БЗ (гипотеза 1).
- `fr-documentation-stepwise.md`, `…-oneshot.md`: уровень 4.x.1 переведён на
  читаемый язык с анти-канцелярит примером, уровень 4.x требует явного субъекта;
  явная реконструкция US/UC при их отсутствии (гипотеза 2 + позитив).
- `fr-documentation-stepwise.md`: в Шаг 4 добавлена сверка каждого требования с
  фактами As-Is/документацией (доп. находка по стенограмме).
- `constraints-documentation-oneshot.md`: вставлен канонический текст пунктов
  6.1.1–6.1.3 (гипотеза 3); добавлен фильтр против ограничений, описывающих
  текущее поведение или дублирующих раздел 4; квота «4-6» смягчена (гипотеза 4).
- `constraints-documentation-stepwise.md`: тот же фильтр добавлен на шаге майнинга
  ограничений (гипотеза 4).
