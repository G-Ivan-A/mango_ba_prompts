...
### Added — Issue #357: валидация активных спринтов бэклога Хаба

- Добавлен протокол сплошной валидации бэклога Хаба
  `docs/analysis/2026-09-03-hub-backlog-sprint-validation.md`: проверены все
  спринты 3–13 и блок отложенных задач `pr-ops/backlog.md` Хаба (v1.53),
  70 связанных issue/PR и итоговые артефакты каждой задачи.
- Зафиксирована исполнимая дельта для Хаба: архивация полностью завершённых
  спринтов 5, 11 и 12; перевод B-098/B-103/B-104 `review` → `DONE`; перевод
  B-081 `TODO` → `ЧАСТИЧНО`; версия `pr-ops/backlog.md` Хаба → 1.54.
- Опровергнуто предположение постановки о завершённости разделения Mango:
  `mango_ba_prompts` остаётся публичным, `ai-ba-playbooks` содержит только
  `README.md` — Спринт 8 не архивируется.
- Добавлен валидатор `scripts/validate_issue_357_hub_backlog_validation.py`:
  сторожит покрытие всех спринтов, абсолютность ссылок-доказательств и
  регистрацию артефакта в карте и changelog.

### Changed — Issue #357

- `pr-ops/artifact-map.md` (v0.7 → v0.8): зарегистрирован протокол валидации
  бэклога Хаба.

### Added — Issue #355: Переработка ФТ BCREQ-1074 (RUN-0067)

- Добавлен прогон `runs/2026/RUN-0067` с ФТ BCREQ-1074 по шаблону BCREQ-1025:
  дельта сведена к детализированной выгрузке (8 полей в заданном порядке,
  CSV/XLSX, телефоны как текст, сумма до копеек) и обработке пустого результата.
- Аналитические комментарии вынесены из тела требований в раздел
  «Вопросы владельцу продукта»; раздел 6.1 явно перечисляет нереализуемое.
- Добавлен валидатор `scripts/validate_issue_355_run.py`, обнаруживаемый
  `make validate-full`.

### Changed — Issue #355

- Прогон RUN-0063 помечен невалидным (`status: invalid`, `superseded_by: RUN-0067`)
  в метаданных, выходном документе, заметках ревью и реестре `runs/README.md`.

### Fixed — Issue #355

- `scripts/validate_pagination_shift.py` сравнивает заголовок цитаты через
  `normalize()` и разворачивает YAML-экранированную кавычку `\"` во frontmatter,
  поэтому заголовки вида «Раздел "Финансы"» больше не дают ложных срабатываний.
...
- Добавлен прогон `runs/2026/RUN-0066/` — переоценка ТЗ task-1099 моделью
  `claude-opus-5` для A/B-сравнения с RUN-0065 (`gpt-5.6-sol`): оценены все 386
  требований (338 `Да`, 34 `Частично`, 14 `Нет`, 0 пустых), 702 атомарные ссылки.
- Номера страниц и § в атомарных ссылках больше не задаются в коде: они читаются
  из frontmatter разделов `kb/processed` в момент генерации отчёта.
- Добавлен валидатор `scripts/validate_issue_351_run.py`: ровно 6 колонок, шкала
  `Да / Частично / Нет / пусто`, сплошная нумерация требований и сверка § ,
  заголовка и страниц каждой ссылки с frontmatter БЗ.
- A/B-сравнение с RUN-0065 зафиксировано в `runs/2026/RUN-0066/logs/ab-comparison.md`.

- Исправлены RUN-0062 и RUN-0065 по issue #349: добавлен единый RCA с
  ресимуляцией поиска по полной БЗ, обязательная эскалация БЗ→web, строгий
  шестиколоночный контракт RUN-0065 и regression validator.
...
- Добавлен [`RUN-0064`](runs/2026/RUN-0064/metadata.yaml): полный RCA ошибок
  предыдущего диалога и уточненные ФТ для управления колонками через шестеренку,
  чекбоксы/множественный выбор, Drag-and-Drop и обратимо включаемый новый вид.
- Трассировка отделяет существующие фильтры, CSV и контекст ЛС от требуемой
  дельты; требования связаны с практиками CBAP и процессами eTOM
  Fulfillment/Assurance.
- Добавлен регрессионный валидатор `scripts/validate_issue_345_run.py`.

### Added — Issue #343: RUN-0063, ФТ по BCREQ-1074

- Добавлен [`RUN-0063`](runs/2026/RUN-0063/metadata.yaml) с шестисекционным
  черновиком ФТ на доработку отчета «Финансы → Расходы на связь»: глоссарий,
  проблема/цели/задачи, верхнеуровневые `FR-XX`, детальные требования и
  сценарии, заглушка НФТ и ограничения.
- Протокол шести последовательных экспертных gate отделяет подтвержденные
  факты от гипотез и открытых вопросов; неподтвержденные решения по данным,
  правам, УПД и формату файла вынесены на human review.
- Добавлен регрессионный валидатор `scripts/validate_issue_343_run.py`.

### Added — Issue #339: RUN-0062, оценка Приложений № 5–7

- Добавлен [`RUN-0062`](runs/2026/RUN-0062/metadata.yaml) с единым L0-отчётом
  по трём XLS-вложениям: Телефония, Quality Control и нефункциональные требования.
  Все таблицы имеют ровно шесть колонок; 87 строк исходной формы переносятся
  непосредственно из XLS, а неподтверждённые выводы помечаются `Нет данных`.
- Добавлены детерминированный генератор
  [`experiments/issue_339/generate_run.py`](experiments/issue_339/generate_run.py)
  и валидатор [`scripts/validate_issue_339_run.py`](scripts/validate_issue_339_run.py),
  который сверяет колонки 1–3 с XLS, контрольные суммы, порядок строк, ширину
  таблиц и адресность ссылок SSOT.
