...
> **Операционка фаундера для автоматизации бизнес-анализа в Mango (телеком).**
...
| Вопрос | Ответ |
| --- | --- |
| Чей это проект? | Проект фаундера для автоматизации процессов бизнес-анализа в конкретной компании — Mango (телеком). |
| Публичный или приватный? | Целевая видимость — **Private** (решение ADR-009 v0.3). Смена видимости — действие фаундера; до неё репозиторий считается публичным, и правила публикации применяются строго. |
| На чём работает? | **GitHub — единственная платформа** (issues, PR, Markdown) + **AI-исполнитель (Конард)** как основной инструмент. |
| Чего здесь нет? | Серверной инфраструктуры (serverless/VPS), мультиагентной системы и внешних сервисов исполнения. Инструментальный слой — не предмет этой операционки. |
| Где публичная методология? | В отдельном репозитории [`ai-ba-playbooks`](https://github.com/G-Ivan-A/ai-ba-playbooks): универсальная агентная система и обезличенные практики для БА. |
| Как практики попадают туда? | **Односторонний неавтоматический** поток `приватный → публичный` с ручным отбором человеком; обратный поток запрещён. Процесс — в [`docs/rfc-hub-integration.md`](docs/rfc-hub-integration.md). |
...
- **Стратегическое направление** — перенос зрелых практик наружу: методология —
  в Хаб, обезличенные плейбуки — в `ai-ba-playbooks`. Оба потока
...
| **Чем это _не_ является?** | Это **не** исследовательский репозиторий и **не** публичная витрина методологии. Исследования — в Хабе (по ссылке), публичные плейбуки — в `ai-ba-playbooks`; здесь остаётся операционка Mango. |
