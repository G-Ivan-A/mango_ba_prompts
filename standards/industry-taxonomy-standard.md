...
3. `kb/industry-taxonomy/registry.json` для canonical node ids below Domain;
...
- использовать этот стандарт и `kb/industry-taxonomy/registry.json`, когда
...
`kb/industry-taxonomy/registry.json`. In `industry_ref`, the existing field
...
`kb/industry-taxonomy/registry.schema.json`. Минимальный YAML-представимый
контракт:
...
`kb/industry-taxonomy/registry.schema.json`. Mapping artifacts MAY use the
...
ДОЛЖЕН быть синхронизирован с `kb/industry-taxonomy/registry.json`:
...
crosswalk or the full `kb/industry-taxonomy/registry.json` registry. It
...
- `kb/industry-taxonomy/registry.json`: canonical below-Domain node ids and
