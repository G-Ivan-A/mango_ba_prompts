...
| R8 | `классифицируется` | Артефакт-требование → Domain→Capability→Feature→Function | 1 → 1..N |
...
`Domain→Capability→Feature→Function`; она дополняет её. Поэтому один
...
Domain→Capability→Feature→Function отвечают на другие вопросы.
...
- [ ] `requirement_level` задан как ортогональная ось для артефактов-требований
      (§2.1) и не заменяет Domain→Capability→Feature→Function.
