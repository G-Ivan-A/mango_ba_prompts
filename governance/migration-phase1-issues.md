...
2. После апрува фаундер/мейнтейнер (у кого есть write-доступ) создаёт 9 Issues,
...
и нельзя превращать в стандарт (E6). Удаление `kb/glossary.md` выполняется
**отдельной задачей после** переноса данных в `standards/` (M-003), а не здесь.

### 📄 Артефакты для создания/изменения

- [ ] Создать каталоги: `prompts/`, `prompts/experiments/`, `prompts/archive/`,
      `standards/`, `kb/`, `docs/`, `docs/adr/`, `docs/audit/`.
- [ ] В каждом созданном каталоге — `.gitkeep` с поясняющим комментарием о
      назначении.
- [ ] Убедиться, что `standards/` создан как отдельный каталог для стандартов
      (глоссарий + контракт классификации).
- [ ] `kb/` сохранить как каталог практик/справочников (не стандарт, E6); не
      трогать `kb/glossary.md` в этой задаче.

### ✅ Готово, когда

- [ ] Все 8 каталогов существуют, каждый с `.gitkeep` и комментарием.
- [ ] `standards/` существует и готов для M-003/M-004.
- [ ] `kb/glossary.md` **не** удалён в этой задаче (удаление — после переноса).
- [ ] Запись добавлена в `CHANGELOG.md` (`## Unreleased`).
