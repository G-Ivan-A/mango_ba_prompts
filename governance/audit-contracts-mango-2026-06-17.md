...
> [`governance/sync-matrix-2026-06-17.md`](sync-matrix-2026-06-17.md); аудит самого
> Хаба — в [`governance/audit-hub-2026-06-17.md`](audit-hub-2026-06-17.md).
>
...
| --- | --- | --- | --- | --- | --- | --- |
| [#003](../docs/adr/003-ba-ontology.md) | Онтология БА (Артефакт↔Процесс↔Операция), граф, 8 состояний ЖЦ | ADR | Глобальный (модель) + локальный (реестр 30 артефактов) | Нет аналога в Хабе | **Передача знаний** | Уникальная практика Mango; кандидат на RFC в Хаб (ФТ-5). |
| [#004](../docs/adr/004-operations-taxonomy.md) | Таксономия 13 операций, маппинг BABOK/ISO 29148, профиль аудита | ADR | Глобальный | Нет аналога; пересекается с PR #246 (методологии) | **Частично** (свериться с PR #246) | Маппинг на BABOK универсален; уникальный набор 13 операций — кандидат на передачу знаний. |
| [#005](../docs/adr/005-artifact-team-naming.md) | Нейминг артефактов/документов + Team Directory | ADR | Частично (схема глобальна, реестр кодов и команды локальны) | Да — Хаб: [`standards/file-naming.md`](https://github.com/G-Ivan-A/hybrid-Intelligence-lab/blob/main/standards/file-naming.md) (про файлы), Mango — про ID артефактов | **Частично** (свериться с file-naming) | Разные предметы: file-naming Хаба — про имена файлов, ADR #005 — про ID артефактов БА. Сверить нейминг файлов-стандартов. |
| [#006](../docs/adr/006-prompt-naming.md) | Нейминг промптов `[домен]-[операция]-[режим].md` | ADR | Глобальный | Нет прямого аналога; родственно `file-naming.md` | **Нет** (локальная конвенция промптов) | Схема промптов специфична для библиотеки промптов; в Хабе нет реестра промптов БА. |
| [#007](../docs/adr/007-kb-standard.md) | Стандарт БЗ (pre-RAG), формат цитирования, синхронизация с глоссарием | ADR | Глобальный (формат) + локальный (структура `kb/`) | Частично — пересекается с PR #242 (Research Memory) и `external-knowledge-integration.md` | **Частично** (свериться с PR #242) | Формат цитирования и pre-RAG универсальны; модель Knowledge Object из Хаба может обогатить ADR #007 — кандидат на RFC. |
| [#008](../docs/adr/008-industry-standards-standard.md) | Отраслевые стандарты: терминология, верификация источников, реестр | ADR | Глобальный (правила) + локальный (реестр источников) | Частично — Хаб: `external-knowledge-integration.md`, `research-memory` (Tier 1) | **Частично** (свериться) | Tier-1/Tier-2 модель Хаба и реестр Mango концептуально совместимы; согласовать терминологию tier. |
| [#009](../docs/adr/009-bcreq-formation-process.md) | Процесс формирования BCREQ: 6 подпроцессов, 3 human gate | ADR | Частично (каркас глобален, числа/операции локальны) | Нет аналога в Хабе | **Передача знаний** | Уникальная практика Mango; кандидат на RFC в Хаб (ФТ-5). |
| [#010](../docs/adr/010-pages-ux.md) | UX GitHub Pages: дерево процессов, только узлы с промптами | ADR | Частично (архитектура данные/отображение глобальна) | Нет аналога в Хабе | **Передача знаний** | Уникальная практика Mango; кандидат на RFC в Хаб (ФТ-5). |

**Примечание по ADR #001/#002.** [`001-prompt-standard.md`](../docs/adr/001-prompt-standard.md)
и [`002-pattern-standard.md`](../docs/adr/002-pattern-standard.md) формализуют
...
| [`governance/artifact-map.md`](artifact-map.md) | навигация | Локальный (адаптация Хаба) | Да — Хаб `governance/artifact-map.md` | **Частично** (Smart Sync) | Уже синхронизирован (см. `source_hub` в frontmatter). |
| [`governance/agent-onboarding-protocol.md`](agent-onboarding-protocol.md) | исполнимый контракт | Локальный | Да — Хаб одноимённый эталон | **Частично** (Smart Sync) | Эталон исполнимого контракта Хаба, адаптирован. |
| [`governance/prompt-debugging-process.md`](prompt-debugging-process.md) | процесс | **Локальный** | Нет аналога | **Передача знаний** | Создан в issue #101; уникальный — кандидат на RFC в Хаб (ФТ-4/ФТ-5). |
| [`governance/rfc-register.md`](rfc-register.md) | реестр | Локальный | Частично — Хаб `knowledge-lifecycle-proposal.md` (lifecycle) | **Частично** (интеграция) | Реестр RFC привязывается к lifecycle Хаба (см. `rfc-process.md`). |
| [`governance/audit-contracts-2026-06-17.md`](audit-contracts-2026-06-17.md) | аудит | Локальный | — | **Нет** | Аудит по issue #101 (отладка промптов), исторический. |
| [`governance/BACKLOG.md`](BACKLOG.md) | трекер | Локальный | Да — Хаб `governance/backlog.md` | **Частично** (Smart Sync) | Локальные открытые вопросы; формат из Хаба. |
| [`governance/migration-manifest.md`](migration-manifest.md), [`migration-phase1-issues.md`](migration-phase1-issues.md), [`migration-issues-registry.md`](migration-issues-registry.md) | миграция | Локальный | — | **Нет** | Снимки миграции из Хаба; исторический контекст. |
| [`governance/session-digests.md`](session-digests.md) | журнал | Локальный | Да — механизм из Хаба | **Частично** (Smart Sync) | Механизм session-digest синхронизирован из Хаба. |
...
| **Интеграция процесса Хаба** | rfc-register → lifecycle Хаба | См. [`rfc-process.md`](rfc-process.md). |
...
оформляются как RFC (см. [`rfc-process.md`](rfc-process.md) и
[`sync-matrix-2026-06-17.md`](sync-matrix-2026-06-17.md)).
