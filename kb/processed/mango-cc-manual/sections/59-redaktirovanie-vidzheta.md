---
id: mango-cc-manual-59-redaktirovanie-vidzheta
doc_code: CC
doc_title: "Контакт-центр MANGO OFFICE - Руководство пользователя"
doc_version: "1.26.23"
section: "3.2"
pdf_section: "3.2"
title: "Редактирование виджета"
pdf_heading: "3.2. Редактирование виджета"
pages: "126-127"
source: kb/sources/mango-cc-manual/CC_manual_1.26.23_compressed.pdf
source_part: "1"
source_pages: "ч.1: 126-127"
source_refs: '[{"source_pdf":"kb/sources/mango-cc-manual/CC_manual_1.26.23_compressed.pdf","part":1,"pages":"126-127","global_pages":"126-127"}]'
extracted_by: "pdfplumber 0.11.10"
token_method: "tiktoken:cl100k_base"
tokens: 411
status: extracted
ai-generated: true
---
# 3.2. Редактирование виджета

> Трассировка: PDF §3.2 · сквозные стр. 126-127 · источники: ч.1 `kb/sources/mango-cc-manual/CC_manual_1.26.23_compressed.pdf` с.126-127.

Пользователь может отредактировать (изменить настройки) любого виджета ранее добавленного на Dashboard.

![Изображение, стр. 126](../images/59-redaktirovanie-vidzheta-1.jpeg)

Чтобы отредактировать виджет, нажмите пиктограмму в правом верхнем углу окна виджета.

![Изображение, стр. 126](../images/59-redaktirovanie-vidzheta-2.png)

После редактирования настроек нажмите кнопку Применить Пользователь может изменить расположение выбранного виджета относительно других

![Изображение, стр. 126](../images/59-redaktirovanie-vidzheta-3.jpeg)

виджетов путем перемещения. Для этого зафиксируйте курсор на пиктограмме в правом верхнем углу окна виджета и перемещайте виджет на свободное поле вкладки Dashboard.

![Изображение, стр. 127](../images/59-redaktirovanie-vidzheta-4.jpeg)

<!-- изображение на стр. 127: байты не извлечены (PyMuPDF недоступен) -->
