---
id: rukovodstvo-polzovatelya-rechevaya-anali-29-klyuchevye-slova
doc_code: RUKOVODSTVOP
doc_title: "Руководство пользователя. Речевая аналитика"
doc_version: "1.26"
section: "6.2.1"
pdf_section: "6.2.1"
title: "Ключевые слова"
pdf_heading: "6.2.1. Ключевые слова"
pages: "44-46"
source: kb/sources/speech-analytics/RECHEVAYA-ANALITIKA_1.26.18.pdf
source_part: "1"
source_pages: "ч.1: 44-46"
source_refs: '[{"source_pdf":"kb/sources/speech-analytics/RECHEVAYA-ANALITIKA_1.26.18.pdf","part":1,"pages":"44-46","global_pages":"44-46"}]'
extracted_by: "pdfplumber 0.11.10"
token_method: "tiktoken:cl100k_base"
tokens: 1069
status: extracted
ai-generated: true
---
# 6.2.1. Ключевые слова

> Трассировка: PDF §6.2.1 · сквозные стр. 44-46 · источники: ч.1 `kb/sources/speech-analytics/RECHEVAYA-ANALITIKA_1.26.18.pdf` с.44-46.

Блок ключевых слов содержит следующие поля: Название тематики. По умолчанию новая тематика именуется Новая тематика. Изменить

![Изображение, стр. 44](../images/29-klyuchevye-slova-1.png)

название можно, щелкнув по пиктограмме . Допускается ввод не более 64 символов. Ключевые слова. При вводе терм в поле "Ключевые слова" производится проверка на наличие каждого слова в словаре распознавания (с учетом словоформ). Если введенное слово отсутствует в словаре, оно выделяется пиктограммой «!». Вероятно, в написании слова имеется ошибка, поэтому оно не может быть распознано. Речевая аналитика предложит исправить слово или добавить его в словарь распознавания. Языковая модель Речевой аналитики регулярно обновляется новыми словами, добавленными пользователями в словарь распознавания.

![Изображение, стр. 44](../images/29-klyuchevye-slova-2.png)

Подробнее о том, как сформулировать поисковый запрос, узнайте здесь. При помощи ИИ (искусственного интеллекта) пользователь может подобрать синонимы и аналогичные выражения к своему списку ключевых слов. Для этого требуется указать несколько ключевых слов и нажать на кнопку «Подобрать слова». После этого в появившемся списке можно выбрать любое слово, и оно автоматически добавится в поле ключевых слов тематики. Лимит на количество запросов синонимов - 8 запросов в час. Также в блоке ИИ доступны следующие действия:

![Изображение, стр. 44](../images/29-klyuchevye-slova-3.png)

– клик по кнопке добавляет все предложенные синонимы в поле ключевых слов тематики.

![Изображение, стр. 44](../images/29-klyuchevye-slova-4.png)

– обновить список синонимов.

![Изображение, стр. 44](../images/29-klyuchevye-slova-5.png)

– удалить все синонимы. Речевая аналитика | v.1.26.18 44

![Изображение, стр. 45](../images/29-klyuchevye-slova-6.png)

| 8 800 555 55 22, mango-office.ru |  |
| --- | --- |
|  | mango@mangotele.com |

![Изображение, стр. 45](../images/29-klyuchevye-slova-7.png)

– отметить предложенные синонимы как подходящие или неподходящие.

![Изображение, стр. 45](../images/29-klyuchevye-slova-8.jpeg)

После ввода ключевых слов следует настроить опции распознавания, кликнув по кнопке Расширенные настройки. Речевая аналитика | v.1.26.18 45

![Изображение, стр. 46](../images/29-klyuchevye-slova-9.png)

| 8 800 555 55 22, mango-office.ru |  |
| --- | --- |
|  | mango@mangotele.com |
