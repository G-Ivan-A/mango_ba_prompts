---
id: rukovodstvo-polzovatelya-rechevaya-anali-86-nastroyka-shemy-raspredeleniya-zvonkov-v
doc_code: RUKOVODSTVOP
doc_title: "Руководство пользователя. Речевая аналитика. ВАТС & Офлайн скоринг"
doc_version: "1.26"
section: "11.4.2"
pdf_section: "11.4.2"
title: "Настройка схемы распределения звонков в Личном кабинете"
pdf_heading: "11.4.2 Настройка схемы распределения звонков в Личном кабинете"
pages: "138-141"
source: kb/sources/speech-analytics/RECHEVAYA-ANALITIKA_VATS-_-Skoring-1.26.18.pdf
source_part: "1"
source_pages: "ч.1: 138-141"
source_refs: '[{"source_pdf":"kb/sources/speech-analytics/RECHEVAYA-ANALITIKA_VATS-_-Skoring-1.26.18.pdf","part":1,"pages":"138-141","global_pages":"138-141"}]'
extracted_by: "pdfplumber 0.11.10"
token_method: "tiktoken:cl100k_base"
tokens: 952
status: extracted
ai-generated: true
---
# 11.4.2. Настройка схемы распределения звонков в Личном кабинете

> Трассировка: PDF §11.4.2 · сквозные стр. 138-141 · источники: ч.1 `kb/sources/speech-analytics/RECHEVAYA-ANALITIKA_VATS-_-Skoring-1.26.18.pdf` с.138-141.

звонков в Личном кабинете Для настройки схемы распределения звонков откройте подраздел “Голосовое меню и распределение звонков” в разделе “Обработка звонков” в Личном кабинете пользователя ВАТС. Панель настройки содержит вкладки: • Настройки для входящих линий • Дополнительные настройки Выберите из списка телефонный̆ номер, подключённый̆ через SIP Trunk, для которого будет настраиваться схема. Далее действуйте согласно инструкции о создании схем переадресации. См. раздел «Обработка звонков» в Справочнике ВАТС. Речевая аналитика. ВАТС & Офлайн скоринг | v.1.26.18 138

![Изображение, стр. 139](../images/86-nastroyka-shemy-raspredeleniya-zvonkov-v-1.png)

| 8 800 555 55 22, mango-office.ru |  |
| --- | --- |
|  | mango@mangotele.com |

![Изображение, стр. 139](../images/86-nastroyka-shemy-raspredeleniya-zvonkov-v-2.png)

Для создания схемы, включающей̆ переадресацию звонков на SIP Trunk кликните по ссылке “не указано”.

![Изображение, стр. 139](../images/86-nastroyka-shemy-raspredeleniya-zvonkov-v-3.jpeg)

В открывшейся форме выберите вкладку SIP Trunk. В выпадающем окне содержится список всех активных транков. При выборе транка в окне отображаются его параметры.

![Изображение, стр. 139](../images/86-nastroyka-shemy-raspredeleniya-zvonkov-v-4.jpeg)

SIP Trunk настроен и может совершать входящие и исходящие звонки. Речевая аналитика. ВАТС & Офлайн скоринг | v.1.26.18 139

![Изображение, стр. 140](../images/86-nastroyka-shemy-raspredeleniya-zvonkov-v-5.png)

| 8 800 555 55 22, mango-office.ru |  |
| --- | --- |
|  | mango@mangotele.com |

Важно: интеграция по SIP Trunk не имеет привязки к сотруднику, таким образом аналитика по сотруднику не доступна. Функционал возможностей Речевой аналитики в этом случае будет ограничен. Речевая аналитика. ВАТС & Офлайн скоринг | v.1.26.18 140

![Изображение, стр. 141](../images/86-nastroyka-shemy-raspredeleniya-zvonkov-v-6.png)

| 8 800 555 55 22, mango-office.ru |  |
| --- | --- |
|  | mango@mangotele.com |
