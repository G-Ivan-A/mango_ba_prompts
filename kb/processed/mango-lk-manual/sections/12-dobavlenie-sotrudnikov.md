---
id: mango-lk-manual-12-dobavlenie-sotrudnikov
doc_code: LK
doc_title: "Виртуальная АТС MANGO OFFICE - Справочник абонента"
doc_version: "1.21"
section: "1.8.1.2"
pdf_section: "1.8.1.2"
title: "Добавление сотрудников."
pdf_heading: "1.8.1.2 Добавление сотрудников."
pages: "15-17"
source: kb/sources/mango-lk-manual/LK_manual_v-121часть-1.pdf
source_part: "1"
source_pages: "ч.1: 15-17"
source_refs: '[{"source_pdf":"kb/sources/mango-lk-manual/LK_manual_v-121часть-1.pdf","part":1,"pages":"15-17","global_pages":"15-17"}]'
extracted_by: "pdfplumber 0.11.10"
token_method: "tiktoken:cl100k_base"
tokens: 580
status: extracted
ai-generated: true
---
# 1.8.1.2. Добавление сотрудников.

> Трассировка: PDF §1.8.1.2 · сквозные стр. 15-17 · источники: ч.1 `kb/sources/mango-lk-manual/LK_manual_v-121часть-1.pdf` с.15-17.

![Изображение, стр. 15](../images/12-dobavlenie-sotrudnikov-1.png)

Шаг 1. Добавление пользователя. Клик по пиктограмме открывает карточку добавления пользователя.

![Изображение, стр. 16](../images/12-dobavlenie-sotrudnikov-2.jpeg)

![Изображение, стр. 16](../images/12-dobavlenie-sotrudnikov-3.jpeg)

Шаг 2. Укажите имя нового пользователя. Указанное имя будет отображаться в списке сотрудников. Шаг 3. Выберите способ приема звонков: • мобильный телефон – укажите номер мобильного телефона сотрудника • SIP (IP-телефония) – sip-адрес и пароль сотрудника генерируются автоматически

![Изображение, стр. 16](../images/12-dobavlenie-sotrudnikov-4.jpeg)

Шаг 4. Подтверждение добавления пользователя и проверка правильности ввода данных. Отредактировать данные пользователя вы можете после прохождения онбординга

![Изображение, стр. 16](../images/12-dobavlenie-sotrudnikov-5.png)

нажав на пиктограмму .

![Изображение, стр. 17](../images/12-dobavlenie-sotrudnikov-6.jpeg)

Чтобы удалить сотрудника, откройте карточку сотрудника, кликните по кнопке Удалить пользователя, подтвердите удаление.

![Изображение, стр. 17](../images/12-dobavlenie-sotrudnikov-7.png)

Клик по пиктограмме прерывает работу обучающего модуля Онбординг.
