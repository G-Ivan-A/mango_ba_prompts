...
scope: kb/sources/web-links
related_issues:
  - "https://github.com/G-Ivan-A/mango_ba_prompts/issues/111"
---

# `kb/sources/web-links/` — источники-ссылки
...
1. Создайте `kb/sources/web-links/<slug>.md` со следующим frontmatter:
...
2. Если страница важна и может измениться/исчезнуть — сохраните в тело файла
   **выжимку** ключевых фактов (как извлечённый раздел в `kb/processed/`), чтобы
   БЗ не зависела от доступности сайта.

3. Цитируйте как обычный источник:
   `[VATS-HELP](kb/sources/web-links/<slug>.md)` и/или прямой `url` из манифеста.
