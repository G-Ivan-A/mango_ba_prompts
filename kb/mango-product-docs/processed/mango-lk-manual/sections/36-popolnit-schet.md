---
id: mango-lk-manual-36-popolnit-schet
doc_code: LK
doc_title: "Виртуальная АТС MANGO OFFICE - Справочник абонента"
doc_version: "1.21"
section: "2.5"
pdf_section: "2.5"
title: "Пополнить счет"
pdf_heading: "2.5 Пополнить счет"
pages: "34"
source: kb/mango-product-docs/sources/mango-lk-manual/LK_manual_v-121часть-1.pdf
source_part: "1"
source_pages: "ч.1: 34"
source_refs: '[{"source_pdf":"kb/mango-product-docs/sources/mango-lk-manual/LK_manual_v-121часть-1.pdf","part":1,"pages":"34","global_pages":"34"}]'
extracted_by: "pdfplumber 0.11.10"
token_method: "tiktoken:cl100k_base"
tokens: 473
status: extracted
ai-generated: true
---
# 2.5. Пополнить счет

> Трассировка: PDF §2.5 · сквозные стр. 34 · источники: ч.1 `kb/mango-product-docs/sources/mango-lk-manual/LK_manual_v-121часть-1.pdf` с.34.

2.5 ПОПОЛНИТЬ СЧЕТ На вкладке Пополнить счет расположена форма для быстрого зачисления средств через платежную систему PayMaster. Укажите сумму платежа, нажмите кнопку Оплатить и действуйте согласно инструкции в открывшейся форме PayMaster.

![Изображение, стр. 34](../images/36-popolnit-schet-1.png)

![Изображение, стр. 34](../images/36-popolnit-schet-2.png)

Также пополнять счет можно иными способами. Выберите наиболее удобный для вас.

![Изображение, стр. 34](../images/36-popolnit-schet-3.png)

![Изображение, стр. 34](../images/36-popolnit-schet-4.png)

Для того, чтобы вам было легче ориентироваться в своих расходах, на каждой странице с выбранным способом оплаты размещен информационный блок "Сколько положить на счет?". Он содержит информацию о расходах за последние 7 и 30 календарных дней соответственно. Для новых клиентов блок "Сколько положить на счет" отображается только на 9-ый день использования продукта. Начиная с 32-го дня выводятся две строки о расходах.
