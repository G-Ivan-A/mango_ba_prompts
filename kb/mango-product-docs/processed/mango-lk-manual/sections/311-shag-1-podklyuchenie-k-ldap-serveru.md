---
id: mango-lk-manual-311-shag-1-podklyuchenie-k-ldap-serveru
doc_code: LK
doc_title: "Виртуальная АТС MANGO OFFICE - Справочник абонента"
doc_version: "1.21"
section: "0"
pdf_section: "4.6.7.5"
title: "Шаг 1. Подключение к LDAP-серверу"
pdf_heading: "Шаг 1. Подключение к LDAP-серверу"
pages: "511-513"
source: kb/mango-product-docs/sources/mango-lk-manual/LK_manual_v-121часть-5.pdf
source_part: "5"
source_pages: "ч.5: 107-109"
source_refs: '[{"source_pdf":"kb/mango-product-docs/sources/mango-lk-manual/LK_manual_v-121часть-5.pdf","part":5,"pages":"107-109","global_pages":"511-513"}]'
extracted_by: "pdfplumber 0.11.10"
token_method: "tiktoken:cl100k_base"
tokens: 475
status: extracted
ai-generated: true
---
# Шаг 1. Подключение к LDAP-серверу

> Трассировка: PDF §4.6.7.5 · сквозные стр. 511-513 · источники: ч.5 `kb/mango-product-docs/sources/mango-lk-manual/LK_manual_v-121часть-5.pdf` с.107-109.

На данной вкладке осуществляется настройка подключения. При подключении по LDAP следует заполнить набор полей:

![Изображение, стр. 512](../images/311-shag-1-podklyuchenie-k-ldap-serveru-1.jpeg)

![Изображение, стр. 512](../images/311-shag-1-podklyuchenie-k-ldap-serveru-2.png)

Поля, отмеченные символом «*», являются обязательными для заполнения. При необходимости использования защищенного подключения следует установить соответствующий флаг и выбрать сертификат. После ввода всех обязательных полей становится доступной кнопка Проверить соединение. При нажатии кнопки выполняется проверка наличия доступа к системе клиента по указанным настройкам. В результате успешного подключения указывается значение «Подключено». Также при успешном подключении на вкладке «Подключение по LDAP» в строке «Статус подключения» отобразится зелёная пиктограмма.

![Изображение, стр. 512](../images/311-shag-1-podklyuchenie-k-ldap-serveru-3.png)

При корректной настройке становится доступным следующий шаг.

![Изображение, стр. 513](../images/311-shag-1-podklyuchenie-k-ldap-serveru-4.jpeg)
