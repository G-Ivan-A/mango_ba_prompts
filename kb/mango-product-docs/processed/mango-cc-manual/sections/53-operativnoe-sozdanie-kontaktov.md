---
id: mango-cc-manual-53-operativnoe-sozdanie-kontaktov
doc_code: CC
doc_title: "Контакт-центр MANGO OFFICE - Руководство пользователя"
doc_version: "1.26.23"
type: "user_manual"
product: "MANGO OFFICE Contact Center"
language: "ru"
topics: ["начало работы","регистрация пользователей","настройки контакт-центра","обработка обращений","исходящие обращения","кампании","API интеграция","требования к оборудованию"]
section: "2.5.10"
pdf_section: "2.5.10"
title: "Оперативное создание контактов"
pdf_heading: "2.5.10. Оперативное создание контактов"
pages: "113-114"
source: kb/mango-product-docs/sources/mango-cc-manual/CC_manual_1.26.23-part-2.pdf
source_part: "2"
source_pages: "ч.2: 12-13"
source_refs: '[{"source_pdf":"kb/mango-product-docs/sources/mango-cc-manual/CC_manual_1.26.23-part-2.pdf","part":2,"pages":"12-13","global_pages":"113-114"}]'
extracted_by: "pdfplumber 0.11.10"
token_method: "tiktoken:cl100k_base"
tokens: 352
status: extracted
ai-generated: true
---
# 2.5.10. Оперативное создание контактов

> Трассировка: PDF §2.5.10 · сквозные стр. 113-114 · источники: ч.2 `kb/mango-product-docs/sources/mango-cc-manual/CC_manual_1.26.23-part-2.pdf` с.12-13.

Вы можете работать с контактами в ходе разговора.

![Изображение, стр. 113](../images/53-operativnoe-sozdanie-kontaktov-1.png)

Чтобы создать новый контакт, нажмите кнопку и выберите действие "Создать контакт":

![Изображение, стр. 114](../images/53-operativnoe-sozdanie-kontaktov-2.png)

<!-- изображение на стр. 114: байты не извлечены (PyMuPDF недоступен) -->

После этого на экране отобразится карточка нового контакта. Чтобы привязать номер к другому контакту, нажмите ссылку "Привязать к существующему". После этого на экране отображается всплывающее окно, позволяющее найти и выбрать нужный контакт. При вводе ФИО контакта работает автоподбор.
