---
id: rukovodstvo-polzovatelya-rechevaya-anali-50-popolnit-schet
doc_code: RUKOVODSTVOP
doc_title: "Руководство пользователя. Речевая аналитика & КАТС"
doc_version: "1.26"
type: "user_manual"
product: "Mango Office"
platform: ["Web"]
language: "ru"
topics: ["речевая аналитика","аналитика звонков","КАТС","скоринг","оценка качества","отчетность"]
section: "6.5"
pdf_section: "6.5"
title: "Пополнить счет"
pdf_heading: "6.5. Пополнить счет"
pages: "94-95"
source: kb/mango-product-docs/sources/speech-analytics/RECHEVAYA-ANALITIKA_c-KATS_v.1.26.18.pdf
source_part: "1"
source_pages: "ч.1: 94-95"
source_refs: '[{"source_pdf":"kb/mango-product-docs/sources/speech-analytics/RECHEVAYA-ANALITIKA_c-KATS_v.1.26.18.pdf","part":1,"pages":"94-95","global_pages":"94-95"}]'
extracted_by: "pdfplumber 0.11.10"
token_method: "tiktoken:cl100k_base"
tokens: 401
status: extracted
ai-generated: true
---
# 6.5. Пополнить счет

> Трассировка: PDF §6.5 · сквозные стр. 94-95 · источники: ч.1 `kb/mango-product-docs/sources/speech-analytics/RECHEVAYA-ANALITIKA_c-KATS_v.1.26.18.pdf` с.94-95.

На вкладке Пополнить счет пополнять счет можно разными способами. Выберите наиболее удобный для вас.

![Изображение, стр. 94](../images/50-popolnit-schet-1.png)

Речевая аналитика & КАТС | v.1.26.18 94

![Изображение, стр. 95](../images/50-popolnit-schet-2.png)

| 8 800 555 55 22, mango-office.ru |  |
| --- | --- |
|  | mango@mangotele.com |

Для того, чтобы вам было легче ориентироваться в своих расходах, на каждой странице с выбранным способом оплаты размещен информационный блок «Сколько положить на счет?». Он содержит информацию о расходах за последние 7 и 30 календарных дней соответственно. Для новых клиентов блок "Сколько положить на счет" отображается только на 9-ый день использования продукта. Начиная с 32-го дня выводятся две строки о расходах.
