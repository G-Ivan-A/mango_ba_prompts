---
id: rukovodstvo-polzovatelya-rechevaya-anali-76-vkladka-zhurnal-deystviy
doc_code: RUKOVODSTVOP
doc_title: "Руководство пользователя. Речевая аналитика & КАТС"
doc_version: "1.26"
type: "user_manual"
product: "Mango Office"
platform: ["Web"]
language: "ru"
topics: ["речевая аналитика","аналитика звонков","КАТС","скоринг","оценка качества","отчетность"]
section: "7.3.3"
pdf_section: "7.3.3"
title: "Вкладка «Журнал действий»"
pdf_heading: "7.3.3. Вкладка «Журнал действий»"
pages: "126-127"
source: kb/mango-product-docs/sources/speech-analytics/RECHEVAYA-ANALITIKA_c-KATS_v.1.26.18.pdf
source_part: "1"
source_pages: "ч.1: 126-127"
source_refs: '[{"source_pdf":"kb/mango-product-docs/sources/speech-analytics/RECHEVAYA-ANALITIKA_c-KATS_v.1.26.18.pdf","part":1,"pages":"126-127","global_pages":"126-127"}]'
extracted_by: "pdfplumber 0.11.10"
token_method: "tiktoken:cl100k_base"
tokens: 549
status: extracted
ai-generated: true
---
# 7.3.3. Вкладка «Журнал действий»

> Трассировка: PDF §7.3.3 · сквозные стр. 126-127 · источники: ч.1 `kb/mango-product-docs/sources/speech-analytics/RECHEVAYA-ANALITIKA_c-KATS_v.1.26.18.pdf` с.126-127.

Данный отчет отображает информацию о действиях пользователей в Личном кабинете. Отчет формируется по указанному периоду. Выберите период или укажите его самостоятельно в полях «с» и «по». При необходимости заполните поле «Поиск». Допускается поиск по: • ФИО сотрудника; • логину вида xxxxxxxxx/name, где: o xxxxxxxxx - цифровой идентификатор; o name – логин пользователя; • IP-адресу; • названию раздела; • названию выполненной операции. внимание Поиск осуществляется по точному совпадению указанных данных. Обращайте внимание на последовательность слов в запросе. Для формирования отчета нажмите Показать. Пример отчета приведен на рисунке ниже.

![Изображение, стр. 126](../images/76-vkladka-zhurnal-deystviy-1.png)

Некоторые действия могут выполняться не сотрудником, а системой (например, отправка письма либо уведомления). В отчете такие действия помечаются как выполненные псевдо- пользователем «Система». Речевая аналитика & КАТС | v.1.26.18 126

![Изображение, стр. 127](../images/76-vkladka-zhurnal-deystviy-2.png)

| 8 800 555 55 22, mango-office.ru |  |
| --- | --- |
|  | mango@mangotele.com |
