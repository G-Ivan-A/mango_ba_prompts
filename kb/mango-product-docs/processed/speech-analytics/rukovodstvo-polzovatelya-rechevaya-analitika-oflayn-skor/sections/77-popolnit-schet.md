---
id: rukovodstvo-polzovatelya-rechevaya-anali-77-popolnit-schet
doc_code: RUKOVODSTVOP
doc_title: "Руководство пользователя. Речевая аналитика. Офлайн скоринг"
doc_version: "1.26"
section: "8.5"
pdf_section: "8.5"
title: "Пополнить счет"
pdf_heading: "8.5. Пополнить счет"
pages: "124-125"
source: kb/mango-product-docs/sources/speech-analytics/RECHEVAYA-ANALITIKA_Skoring_Rukovodstvo-polzovatelya_v.1.26.15.pdf
source_part: "1"
source_pages: "ч.1: 124-125"
source_refs: '[{"source_pdf":"kb/mango-product-docs/sources/speech-analytics/RECHEVAYA-ANALITIKA_Skoring_Rukovodstvo-polzovatelya_v.1.26.15.pdf","part":1,"pages":"124-125","global_pages":"124-125"}]'
extracted_by: "pdfplumber 0.11.10"
token_method: "tiktoken:cl100k_base"
tokens: 413
status: extracted
ai-generated: true
---
# 8.5. Пополнить счет

> Трассировка: PDF §8.5 · сквозные стр. 124-125 · источники: ч.1 `kb/mango-product-docs/sources/speech-analytics/RECHEVAYA-ANALITIKA_Skoring_Rukovodstvo-polzovatelya_v.1.26.15.pdf` с.124-125.

На вкладке Пополнить счет пополнять счет можно разными способами. Выберите наиболее удобный для вас.

![Изображение, стр. 124](../images/77-popolnit-schet-1.png)

Речевая аналитика. Скоринг | v.1.26.15 124

![Изображение, стр. 125](../images/77-popolnit-schet-2.png)

| 8 800 555 55 22, mango-office.ru |  |
| --- | --- |
|  | mango@mangotele.com |

Для того, чтобы вам было легче ориентироваться в своих расходах, на каждой странице с выбранным способом оплаты размещен информационный блок «Сколько положить на счет?». Он содержит информацию о расходах за последние 7 и 30 календарных дней соответственно. Для новых клиентов блок "Сколько положить на счет" отображается только на 9-ый день использования продукта. Начиная с 32-го дня выводятся две строки о расходах.
