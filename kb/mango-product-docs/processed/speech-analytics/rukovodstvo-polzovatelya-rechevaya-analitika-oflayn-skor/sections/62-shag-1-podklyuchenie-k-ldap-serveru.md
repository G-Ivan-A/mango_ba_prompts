---
id: rukovodstvo-polzovatelya-rechevaya-anali-62-shag-1-podklyuchenie-k-ldap-serveru
doc_code: RUKOVODSTVOP
doc_title: "Руководство пользователя. Речевая аналитика. Офлайн скоринг"
doc_version: "1.26"
section: "0"
pdf_section: "7.5"
title: "Шаг 1. Подключение к LDAP-серверу"
pdf_heading: "Шаг 1. Подключение к LDAP-серверу"
pages: "102"
source: kb/mango-product-docs/sources/speech-analytics/RECHEVAYA-ANALITIKA_Skoring_Rukovodstvo-polzovatelya_v.1.26.15.pdf
source_part: "1"
source_pages: "ч.1: 102"
source_refs: '[{"source_pdf":"kb/mango-product-docs/sources/speech-analytics/RECHEVAYA-ANALITIKA_Skoring_Rukovodstvo-polzovatelya_v.1.26.15.pdf","part":1,"pages":"102","global_pages":"102"}]'
extracted_by: "pdfplumber 0.11.10"
token_method: "tiktoken:cl100k_base"
tokens: 423
status: extracted
ai-generated: true
---
# Шаг 1. Подключение к LDAP-серверу

> Трассировка: PDF §7.5 · сквозные стр. 102 · источники: ч.1 `kb/mango-product-docs/sources/speech-analytics/RECHEVAYA-ANALITIKA_Skoring_Rukovodstvo-polzovatelya_v.1.26.15.pdf` с.102.

На данной вкладке осуществляется настройка подключения. При подключении по LDAP следует заполнить набор полей:

![Изображение, стр. 102](../images/62-shag-1-podklyuchenie-k-ldap-serveru-1.png)

Поля, отмеченные символом «*», являются обязательными для заполнения. При необходимости использования защищенного подключения следует установить соответствующий флаг и выбрать сертификат. После ввода всех обязательных полей становится доступной кнопка Проверить соединение. При нажатии кнопки выполняется проверка наличия доступа к системе клиента по указанным настройкам. В результате успешного подключения указывается значение «Подключено». Также при успешном подключении на вкладке «Подключение по LDAP» в строке «Статус подключения» отобразится зелёная пиктограмма.

![Изображение, стр. 102](../images/62-shag-1-podklyuchenie-k-ldap-serveru-2.png)

При корректной настройке становится доступным следующий шаг.
