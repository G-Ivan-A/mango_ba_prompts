---
id: integration-bitrix24-80-kak-otpravit-sms-rassylku
doc_code: INTEGRATIONB
doc_title: "Интеграция Виртуальной АТС и Битрикс24. Инструкция по настройке"
doc_version: "03.03.2026"
type: "integration_guide"
product: "Mango Office"
platform: ["Web"]
language: "ru"
topics: ["интеграция","Битрикс24","CRM","ВАТС","настройка","синхронизация"]
section: "0"
pdf_section: "2.6"
title: "Как отправить sms-рассылку"
pdf_heading: "Как отправить sms-рассылку"
pages: "71-74"
source: kb/mango-product-docs/sources/integration-bitrix24/Mango_office_integration_Bitrix24.pdf
source_part: "1"
source_pages: "ч.1: 71-74"
source_refs: '[{"source_pdf":"kb/mango-product-docs/sources/integration-bitrix24/Mango_office_integration_Bitrix24.pdf","part":1,"pages":"71-74","global_pages":"71-74"}]'
extracted_by: "pdfplumber 0.11.10"
token_method: "tiktoken:cl100k_base"
tokens: 513
status: extracted
ai-generated: true
---
# Как отправить sms-рассылку

> Трассировка: PDF §2.6 · сквозные стр. 71-74 · источники: ч.1 `kb/mango-product-docs/sources/integration-bitrix24/Mango_office_integration_Bitrix24.pdf` с.71-74.

Чтобы сделать рассылку, в Битрикс24 следует: 1) выберите "Маркетинг"; 2) выберите "SMS-рассылка":

![Изображение, стр. 71](../images/80-kak-otpravit-sms-rassylku-1.jpeg)

Интеграция Виртуальной АТС MANGO OFFICE и Битрикс24 | Версия от 03.03.2026 3) выберите шаблон рассылки или создайте новый:

![Изображение, стр. 72](../images/80-kak-otpravit-sms-rassylku-2.jpeg)

4) при настройке рассылки выберите приложение MANGO OFFICE для отправки SMS:

![Изображение, стр. 72](../images/80-kak-otpravit-sms-rassylku-3.png)

Интеграция Виртуальной АТС MANGO OFFICE и Битрикс24 | Версия от 03.03.2026 5) настройте остальные параметры рассылки, выберите список Клиентов для рассылки. Сохраните настройки и выберите расписание для отправки:

![Изображение, стр. 73](../images/80-kak-otpravit-sms-rassylku-4.png)

После отправки SMS, информация сохранится в карточке:

![Изображение, стр. 73](../images/80-kak-otpravit-sms-rassylku-5.png)

Интеграция Виртуальной АТС MANGO OFFICE и Битрикс24 | Версия от 03.03.2026
