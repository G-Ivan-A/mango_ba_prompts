---
id: integration-bitrix24-190-b-nastroyka-otobrazheniya-kolonok-v-spis
doc_code: INTEGRATIONB
doc_title: "Интеграция Виртуальной АТС и Битрикс24. Инструкция по настройке"
doc_version: "03.03.2026"
section: "0"
pdf_section: "—"
title: "Б. Настройка отображения колонок в списке лидов, сделок, контактов"
pdf_heading: "Б. Настройка отображения колонок в списке лидов, сделок, контактов"
pages: "172-174"
source: kb/mango-product-docs/sources/integration-bitrix24/Mango_office_integration_Bitrix24.pdf
source_part: "1"
source_pages: "ч.1: 172-174"
source_refs: '[{"source_pdf":"kb/mango-product-docs/sources/integration-bitrix24/Mango_office_integration_Bitrix24.pdf","part":1,"pages":"172-174","global_pages":"172-174"}]'
extracted_by: "pdfplumber 0.11.10"
token_method: "tiktoken:cl100k_base"
tokens: 573
status: extracted
ai-generated: true
---
# Б. Настройка отображения колонок в списке лидов, сделок, контактов

> Трассировка: PDF §— · сквозные стр. 172-174 · источники: ч.1 `kb/mango-product-docs/sources/integration-bitrix24/Mango_office_integration_Bitrix24.pdf` с.172-174.

контактов Чтобы настроить отображение колонок в списке "Контакты" \ "Лиды \ "Контакты", в вашем Битрикс24 следует: 1) откройте нужный раздел: "Контакты" \ "Лиды \ "Контакты;

![Изображение, стр. 172](../images/190-b-nastroyka-otobrazheniya-kolonok-v-spis-1.png)

2) нажмите на кнопку "Шестеренка" в заголовке списка;

![Изображение, стр. 172](../images/190-b-nastroyka-otobrazheniya-kolonok-v-spis-2.jpeg)

3) настройте отображение колонок: - активируйте "галочки" в названии тех колонок, которые надо показать (например, в колонках "ДКТ:…"); - деактивируйте "галочки" в названии тех колонок, которые надо скрыть (например, в колонке "Кем изменен"); 4) нажмите кнопку "Применить"; Интеграция Виртуальной АТС MANGO OFFICE и Битрикс24 | Версия от 03.03.2026

![Изображение, стр. 173](../images/190-b-nastroyka-otobrazheniya-kolonok-v-spis-3.png)

5) будут отображены выбранные вами колонки (например, в колонках "ДКТ:…").

![Изображение, стр. 173](../images/190-b-nastroyka-otobrazheniya-kolonok-v-spis-4.jpeg)

Интеграция Виртуальной АТС MANGO OFFICE и Битрикс24 | Версия от 03.03.2026
