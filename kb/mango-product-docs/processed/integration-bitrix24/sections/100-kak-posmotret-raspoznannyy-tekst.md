---
id: integration-bitrix24-100-kak-posmotret-raspoznannyy-tekst
doc_code: INTEGRATIONB
doc_title: "Интеграция Виртуальной АТС и Битрикс24. Инструкция по настройке"
doc_version: "03.03.2026"
section: "0"
pdf_section: "2.10"
title: "Как посмотреть распознанный текст"
pdf_heading: "Как посмотреть распознанный текст"
pages: "86-87"
source: kb/mango-product-docs/sources/integration-bitrix24/Mango_office_integration_Bitrix24.pdf
source_part: "1"
source_pages: "ч.1: 86-87"
source_refs: '[{"source_pdf":"kb/mango-product-docs/sources/integration-bitrix24/Mango_office_integration_Bitrix24.pdf","part":1,"pages":"86-87","global_pages":"86-87"}]'
extracted_by: "pdfplumber 0.11.10"
token_method: "tiktoken:cl100k_base"
tokens: 267
status: extracted
ai-generated: true
---
# Как посмотреть распознанный текст

> Трассировка: PDF §2.10 · сквозные стр. 86-87 · источники: ч.1 `kb/mango-product-docs/sources/integration-bitrix24/Mango_office_integration_Bitrix24.pdf` с.86-87.

Чтобы вывести на экран распознанный текст того или иного разговора, в Битрикс24 следует: 1) открыть лид; 2) кликнуть на ссылку "Показать расшифровку звонка":

![Изображение, стр. 86](../images/100-kak-posmotret-raspoznannyy-tekst-1.png)

3) посмотреть текст Речевой аналитики:

![Изображение, стр. 86](../images/100-kak-posmotret-raspoznannyy-tekst-2.png)

Интеграция Виртуальной АТС MANGO OFFICE и Битрикс24 | Версия от 03.03.2026
