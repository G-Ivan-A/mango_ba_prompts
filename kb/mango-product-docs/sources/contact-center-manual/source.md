...
для будущих источников. Один каталог `kb/mango-product-docs/sources/<slug>/` = один документ.
...
1. Положите файл сюда: `kb/mango-product-docs/sources/contact-center-manual/CC_manual_1.26.23.pdf`.
2. При необходимости обновите поля выше (`doc_version`, `web_url`).
3. Запустите тот же конвейер в «настоящий» каталог результата:

   ```bash
   make kb-extract SRC=kb/mango-product-docs/sources/contact-center-manual/CC_manual_1.26.23.pdf \
                   OUT=kb/mango-product-docs/processed/contact-center-manual \
                   CODE=CC TITLE="Контакт-центр MANGO OFFICE" VERSION=1.26.23
   ```

4. Проверьте `kb/mango-product-docs/processed/contact-center-manual/index.md` и `make kb-validate`.
